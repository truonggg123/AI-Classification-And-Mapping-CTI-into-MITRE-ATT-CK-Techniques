...
def run_ablation_study(target_dataset='cti_to_mitre'):
    ...
    print("  Tuning Range: alpha in [0.01, 0.20] with Step Size = 0.01 (20 Ratios + Baseline)")
    print("=" * 80)
    ...
    # ── Step 3: Define 21 Uniform Configs (alpha in [0.01, 0.20]) ──────────
    configs = [
        {
    ...
    alpha_values = [round(a, 2) for a in np.arange(0.01, 0.201, 0.01)]
    ...
    pd.DataFrame(rows).to_csv(out_dir / 'cybereda_ablation_results.csv', index=False, encoding='utf-8')
    print(f"\n  [SAVED] {out_dir / 'cybereda_ablation_results.csv'}")
